...
graph_run_duration_seconds = Histogram(
    "graph_run_duration_seconds", "Full LangGraph run duration (s)", ["mode"],
    buckets=[1, 3, 5, 10, 15, 20, 30, 60, 120],
)
graph_run_total = Counter(
    "graph_run_total", "Total LangGraph runs", ["mode", "status"]
)
provider_call_duration_seconds = Histogram(
    "provider_call_duration_seconds", "External provider call duration (s)", ["provider"],
)
...
cache_lookups_total = Counter(
    "cache_lookups_total", "Response cache lookups", ["result"]  # hit | miss
)
